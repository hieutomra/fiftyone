        ...
        if ctx.params["compute"] == "all":
            ...
            for issue in ISSUE_TYPES:
                if not (ctx.dataset.has_sample_field(issue)):
                    ...
                    if issue == "duplicates":
                        # TODO: Implement this
                        # This requires running an operator to get user input so
                        # we may want to run this first? Another possibility is
                        # to assume if someone runs "Execute All" then they consent
